...
class SongDownloader < Downloader
  ...
  def self.download(url, counter, settings, number_of_tries = 1)
    ...
    Logger.progress(song, song.counter)
    return Logger.maximum_number_of_tries if number_of_tries > MAXIMUM_NUMBER_OF_TRIES
    song.download
  rescue
    Logger.retry(song.url, song.name, number_of_tries)
    download(url, counter, settings, number_of_tries + 1)
  end

  def download
    Logger.downloading(self)
    YoutubeDL.download url, format: 'm4a', output: settings.filepath(name)
    Logger.finished_downloading
  end
  ...
  def find_youtube_video(name)
    ...
    puts Logger.found_no_results(@url)
  end
end
